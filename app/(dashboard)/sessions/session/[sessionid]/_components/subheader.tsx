...
export default function SubheaderSES({id,completedFields,isComplete, issecond, isfirst}: SubheadrProps) {
  return (
    <div className="flex justify-between items-center p-2 border rounded-md mb-2">
                ...
                {(isComplete && issecond) && (
                  <div>
                    <Actions disabled={false} sessionid={id} />
                  </div>
                )}
                 {(isComplete && isfirst) && (
                  <div>
                    <FirstActionsEXP disabled={false} sessionid={id} />
                  </div>
                )}
              </div>
  )
