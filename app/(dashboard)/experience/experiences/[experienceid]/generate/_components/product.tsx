...
const router = useRouter();
  ...
  async function aiRun() {
    ...
    const prompt =   `Taking into account the Evaluation Criteria on the Specified Performance of each of the Competencies which are as follows ${evaluationcriteria} and Thematic Fields built in this "Learning Experience" according to the National Curriculum of Regular Basic Education (DCN-EBR) of the Ministry of Education of Peru which are ${thematicfields}, mention 1 to 3 Products that we can develop in the classroom. Generate everything in Spanish.
    `;
    ...
    try{
      ...
      const res = await axios.patch(`/api/experience/${experienceid}/product`, { text: JSON.stringify(text) });
      setisLoading(false);
      console.log(res)
