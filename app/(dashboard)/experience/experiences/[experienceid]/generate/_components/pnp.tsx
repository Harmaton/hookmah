...
interface PNPFormProps {
  initialData: Experience;
  age: {name: string} | null;
  experienceid: string;
}

export const PNPForm = ({
  initialData,
  experienceid,
  age
}: PNPFormProps) => {
...
const router = useRouter();
  ...
  async function aiRun() {
    ...
    const prompt =   ` Generate the needs and problems of students according to the age that the teacher entered of this students. The age is (${age?.name})
    ...
    try{
      ...
      const res =  await axios.patch(`/api/experience/${experienceid}/pnp`, { text: JSON.stringify(text) });
      setisLoading(false);
      console.log(res)
  ...
  return (
    <div className="rounded-md p-4 border-red-200">
      ...
      {!isLoading && (
        ...
        >
          {initialData.pnpcharacteristics || "Sin características"}
        </p>
      )}
