...
export const DepartmentForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      toast.success("Departmento actualizado");
      toggleEdit();
      router.refresh();
    } catch {
  ...
  const selectedOption = options ? options.find((option) => option.value === initialData.departmentId) : null;
    ...
    <div className="mt-6 bg-transparent rounded-md p-4">
      ...
      {!isEditing && (
        <p className={cn(
          ...
          {selectedOption?.label || "Ninguna departmento"}
        </p>
      )}
      {isEditing && (
        <Form {...form}>
